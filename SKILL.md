...
- **Limpiar para el siguiente cliente (6 ago 2026, noche):** JC reportó que terminado el correo **y** el WhatsApp la pantalla quedaba con los datos del cliente y no había forma de vaciarla. Causa: el botón "Enviar a otro cliente" solo vivía en el panel de éxito, que **solo se muestra con `canal === 'correo'`** — al pasarse a WhatsApp quedaba la franja compacta, sin ese botón, y la única salida era F5. Fix: (1) la franja `bannerExito()` lleva el mismo botón; (2) **"🧹 Limpiar" permanente** en la barra de "Envío de pólizas", junto a "+ Agregar viajero", visible solo si `hayAlgoQueLimpiar()` (hay envío hecho, teléfono, destinatario, o algún viajero con datos/archivos) — sirve además cuando NO se envió nada (PDF equivocado, cliente arrepentido); (3) los **tres** caminos pasan por `pedirLimpiar()`, que confirma y enumera lo que se va (`resumenDeLoQueSeBorra()` → "2 asegurados cargados, 6 archivos, el destinatario y el teléfono") — también el del panel de éxito, porque el WhatsApp puede estar todavía sin mandar y borrar sin preguntar obligaría a recargar los PDF. La ventana modal se generalizó en `confirmar(o)` (icono, títulos, textos, `rojo` para el tono destructivo, `onSi`), reusada por el agregar-asegurado y el limpiar. Verificado: pantalla vacía sin botón, con datos aparece, franja con botón tras pasar a WhatsApp, "No" conserva todo, "Sí" deja stepper 1-2-3 y todos los campos vacíos.
- **Nombre del asegurado bajo su póliza + el teléfono como bloque (6 ago 2026, tarde):** dos pedidos de JC sobre el panel de WhatsApp. (1) **Comodín `{Asegurados}`** (nuevo, reemplaza a `📄 *Póliza N° {Poliza}*` en las 2 plantillas por defecto): con un asegurado escribe `📄 *Póliza N° X*` y el nombre debajo; con dos o más, encabezado `📄 *Pólizas emitidas*` y cada número con su nombre debajo, separados por línea en blanco. **El nombre va TAL COMO viene del INS** (mayúscula, apellidos primero) — decisión de JC: es lo que el cliente reconoce de su documento y evita acomodar mal un apellido compuesto. `{Poliza}` **se conserva** (solo los números unidos) para no romper las plantillas ya guardadas en `localStorage` por cualquier agente. El panel muestra una fila editable por asegurado (N° de póliza + Nombre), autocompletada de los viajeros y congelada al primer toque manual (`asegManual`); sin viajeros deja una fila vacía para "directa". (2) **El teléfono como bloque destacado** (detalle en "app.js — UI"). 🔴 **BUG ATRAPADO EN LA VERIFICACIÓN, no llegó a prod:** al cambiar el ejemplo del campo de `506 8888 8888` a `8888 8888`, el link salía con `phone=88221348` **sin código de país** y WhatsApp habría abierto un número inválido. `buildLink()` ahora antepone `506` cuando vienen exactamente 8 dígitos y respeta cualquier otra cantidad (ya trae código, o es de otro país). Verificado: 8 dígitos → 506+, con 506 → intacto, 10 dígitos extranjeros → intactos, vacío → vacío.
- **Multi-viajero en WhatsApp + confirmación al agregar asegurado (6 ago 2026):** dos ajustes pedidos por JC. (1) 🔴 **FIX de lo entregado el 5 ago:** `syncEnvio()` llenaba `state.poliza` **una sola vez** (`if (pol && !state.poliza)`), así que al entrar un segundo asegurado el mensaje de WhatsApp seguía mostrando solo la póliza del primero, mientras el correo sí llevaba las dos adjuntas. Ahora el número **sigue** a los viajeros (`if (!polizaManual) state.poliza = polizasAuto()`) y se congela solo si el agente lo escribe a mano; **vaciar el campo devuelve el autocompletado**. Además, corregir una póliza a mano en la tarjeta del viajero refresca el input de WhatsApp sin `render()` (para no perder el foco al escribir). Verificado: 2 viajeros → "A y B", 3 → "A, B y C", texto manual respetado al agregar más. (2) **Ventana de confirmación antes de agregar otro asegurado** (detalle en "app.js — UI"). Se descartó el `confirm()` nativo por no poder mostrar a quién ya se cargó.
- **Confirmación visual + póliza en WhatsApp (5 ago 2026):** tres ajustes pedidos por JC, con previo de diseño aprobado antes de tocar código. (1) **Panel de éxito** al enviar el correo, en el lugar de los botones (detalle en "app.js — UI"); JC eligió esta variante sobre la franja discreta y el modal. (2) **Íconos de las 2 plantillas de WhatsApp:** el mundo pasa a avión y el avión a check — `emitida` queda "Listo {Nombre} ✈️ … ✅ *¡Su póliza está lista!*" y `directa` "…Asistencia. ✅ … ¡Buen viaje! ✈️". (3) **Comodín `{Poliza}`** en las dos plantillas (detalle en "wa.js"). Se descartó por ahora el botón que baja los 5 documentos juntos para arrastrarlos al chat. (4) **Registro de cambios al pie de LAS DOS caras** — `<details>` con "Última actualización: <fecha>" y el historial en lenguaje de usuario, lo más reciente arriba: al final de `agente/index.html` (historial técnico-de-uso de la consola) y dentro del `<footer>` de la pública `index.html`, debajo del bloque de copyright (historial en lenguaje de cliente; JC lo aprobó sabiendo que lo ve el asegurado). 🔴 **Regla nueva de JC (5 ago 2026, aplica a TODAS sus apps):** cada update se anota en el pie de la propia aplicación, no solo en el commit o el SKILL — JC no lee commits. Las fechas son reales, reconstruidas del historial: no inventar versiones. 🔴 **Los adjuntos NO se pueden mandar por WhatsApp desde la app:** el esquema `web.whatsapp.com/send` solo transporta texto y no tiene parámetro de archivos; lo único que manda documentos por programa es la WhatsApp Business Cloud API de Meta (cuenta de empresa verificada + servidor propio + plantillas aprobadas por Meta + cobro por conversación), incompatible con esta app sin backend. No volver a proponerlo como si fuera cuestión de código. Verificado en preview: 5 casos de `buildLink` (los 2 mensajes con póliza, sin póliza la línea desaparece sin dejar hueco, dos pólizas unidas con "y", host `web.whatsapp.com`), envío simulado con la capa de red interceptada (1 sola llamada a Gmail, botón Enviar ausente tras enviar, franja al pasar a WhatsApp, reset limpio) y 12/12 del selftest.

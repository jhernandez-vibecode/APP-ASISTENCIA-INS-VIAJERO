...
**Comodín `{Poliza}` (5 ago 2026, corregido el 6 ago):** el número de póliza va en el cuerpo del mensaje (`📄 *Póliza N° {Poliza}*`, línea propia después del saludo). Se autocompleta con el/los número(s) del PDF cargado (`unir()` los une en español: "A, B y C") y **se puede escribir a mano** — indispensable en `directa`, donde el cliente compró por su cuenta y el agente no tiene el PDF. Si el campo queda vacío, `VWa.conPoliza()` borra **la línea entera** y colapsa el salto sobrante: nunca sale un "📄 Póliza N°" a medias. Ojo: una plantilla vieja guardada en `localStorage` no trae el comodín — hay que darle "Restaurar" para que aparezca.
